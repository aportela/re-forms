...
return [
    ...
    \aportela\DatabaseWrapper\DB::class => function (ContainerInterface $container) {
        ...
        $logger = $container->get(\ReForms\Logger\DBLogger::class);
        // main object
    ...
    \ReForms\Logger\DefaultLogger::class => function (ContainerInterface $container) {
        $settings = $container->get('settings')['logger'];
        $logger = new \ReForms\Logger\DefaultLogger($settings['channels']['default']['name']);
    ...
    \ReForms\Logger\DBLogger::class => function (ContainerInterface $container) {
        $settings = $container->get('settings')['logger'];
        $logger = new \ReForms\Logger\DBLogger($settings['channels']['database']['name']);
        $logger->pushProcessor(new \Monolog\Processor\UidProcessor());
    ...
    \ReForms\Logger\InstallerLogger::class => function (ContainerInterface $container) {
        $settings = $container->get('settings')['logger'];
        $logger = new \ReForms\Logger\InstallerLogger($settings['channels']['installer']['name']);
        $logger->pushProcessor(new \Monolog\Processor\UidProcessor());
